...
export const commerceTools = [
  {
    ...
    async handler({ source, retailPence, costPence, tenantId }) {
      const res = await apiClient()("/commerce/price-breakdown", {
        method: "POST",
  ...
  // ── Hosted shops (admin) ──────────────────────────────────────────
  {
    ...
    async handler() {
      const res = await apiClient()("/admin/hosted-shops");
  ...
  },
  {
    ...
    async handler({ tenantId, name, domain }) {
      const res = await apiClient()("/admin/hosted-shops", {
        method: "POST",
  ...
  },
  {
    ...
    async handler({ siteId }) {
      const res = await apiClient()(`/admin/hosted-shops/${siteId}/provision`, { method: "POST" });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  },
  {
    ...
    inputSchema: { type: "object", required: ["siteId"], properties: { siteId: { type: "string" } } },
    async handler({ siteId }) {
      const res = await apiClient()(`/admin/hosted-shops/${siteId}/go-live`, { method: "POST" });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  },
  ...
  // ── Resell payouts (admin) ────────────────────────────────────────
  {
    ...
    async handler({ status } = {}) {
      ...
      const res = await apiClient()(`/admin/resell-payouts${qs}`);
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  },
  {
    ...
    async handler({ id }) {
      const res = await apiClient()(`/admin/resell-payouts/${id}/settle`, { method: "POST" });
      const d = await res.json();
  ...
  // ── Fee rates (admin, per tenant) ─────────────────────────────────
  {
    ...
    async handler({ tenantId }) {
      const res = await apiClient()(`/admin/tenants/${tenantId}/fee-rates`);
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  },
  {
    ...
    async handler({ tenantId, resellTenantSharePct, ownPlatformFeePct }) {
      const body = {};
      ...
      const res = await apiClient()(`/admin/tenants/${tenantId}/fee-rates`, { method: "PATCH", body: JSON.stringify(body) });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
  },
  ...
  // ── Assortment (admin, per tenant) ────────────────────────────────
  {
    ...
    inputSchema: { type: "object", required: ["tenantId"], properties: { tenantId: { type: "string" } } },
    async handler({ tenantId }) {
      const res = await apiClient()(`/admin/tenants/${tenantId}/assortment`);
      const d = await res.json();
      return d.items ?? d;
    },
  },
  {
    ...
    async handler({ tenantId, catalogProductId }) {
      const res = await apiClient()(`/admin/tenants/${tenantId}/assortment`, {
        method: "POST",
  ...
  },
  {
    ...
    async handler({ tenantId, catalogProductId }) {
      const res = await apiClient()(`/admin/tenants/${tenantId}/assortment/${catalogProductId}`, { method: "DELETE" });
      if (!res.ok) throw new Error(await res.text());
      return { ok: true };
    },
  },
  ...
  // ── Catalogue browse (admin) ──────────────────────────────────────
  {
    ...
    async handler({ sector, brand, q, tenantId } = {}) {
      const params = new URLSearchParams();
      if (sector) params.set("sector", sector);
      if (brand) params.set("brand", brand);
      if (q) params.set("q", q);
      if (tenantId) params.set("tenantId", tenantId);
      const res = await apiClient()(`/admin/catalogue?${params.toString()}`);
      const d = await res.json();
      return d.items ?? d;
    },
  },
